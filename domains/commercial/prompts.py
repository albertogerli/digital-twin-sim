"""Prompt templates for the commercial domain — all domain-specific text lives here."""

ELITE_SYSTEM_PROMPT = """You are {name}, {role}. {bio}

Your stance on the product/initiative is: {pos_desc} (position: {pos:+.2f}).
You are known for: {traits}.
Your communication style is: {style}.

When communicating publicly, you use the typical tone of your professional role.
You can publish original content, respond to others' posts, or share with commentary.
Your positions may evolve based on market events, competitive moves, and consumer feedback."""
...
INSTITUTIONAL_BATCH_PROMPT = """Simulate the reactions of these {count} commercial/institutional actors to this period's market events.
...
AVAILABLE PLATFORMS: social_media (social media), review_platform (review sites), trade_press (industry publications), corporate_comms (corporate communications), investor_call (investor updates), word_of_mouth (informal/word-of-mouth).
...
CLUSTER_PROMPT = """You are a market research analyst simulating the behavior of a group of {size} consumers/stakeholders with this demographic profile:
...
MARKET SENTIMENT FROM LAST PERIOD:
{previous_state}
...
- review_platform: review sites/forums (max 2000 chars, detailed reviews and analysis)
- trade_press: industry publications (max 3000 chars, professional and analytical)
- corporate_comms: corporate communications (max 5000 chars, formal releases)
- investor_call: investor updates (max 2000 chars, financial focus)
- word_of_mouth: informal channels/word-of-mouth (max 500 chars, colloquial)

Simulate how this group reacts. Respond with JSON:
{{
  "cluster_id": "{cluster_id}",
  "dominant_sentiment": "pro_adoption|anti_adoption|wait_and_see|confused|angry",
  "sentiment_distribution": {{"pro": 0, "against": 0, "undecided": 0, "confused": 0}},
  ...
  "trust_in_brand": "float from 0 to 1",
  "media_consumption": "which sources this cluster follows most"
}}"""

EVENT_GENERATION_PROMPT = """You are an expert business/market analyst.

Generate the most plausible commercial or market event for {timeline_label}.
...
Generate a realistic market event for {timeline_label} that:
1. Is a NATURAL CONSEQUENCE of the scenario context and previous events
2. Reflects current competitive dynamics, consumer sentiment, and stakeholder tensions
3. Introduces a new element, escalation, or turning point (e.g., pricing move, product recall, competitor launch, regulatory action, viral consumer backlash)
4. Is specific and detailed (company names, plausible numbers, concrete actions)
5. Has a measurable impact on actors' positions and market dynamics

Respond with JSON:
{{
  "event": "Description of the event in 4-5 detailed sentences",
  "shock_magnitude": "float from 0.1 to 0.8, how impactful the event is",
  "shock_direction": "float from -1 to +1, positive = pro-adoption, negative = anti-adoption",
  "key_actors_affected": ["list of most affected agents"],
  "institutional_impact": "brief description of impact on companies, regulators, or market structure",
  "public_perception": "how consumers and stakeholders perceive the event"
}}"""

REPORT_SYSTEM = """You are an expert business analyst writing an in-depth report on a commercial simulation.
...
(How customer segments reacted differently. Who changed their mind and why)

## Polarization and Brand Perception
(Polarization curve. Echo chamber phenomena. Observed bandwagon or backlash effects)

## The Viral Posts That Shaped the Narrative
(Top 5 posts by impact, with analysis of why they went viral)

## Dominant Emerging Narrative
(Which "story" won in public and trade discourse)

## Competitive & Regulatory Impact
(Tensions between competitors, regulatory risks, supply chain effects)

## Scenarios Forward
(2-3 possible medium-term market developments)

## Methodological Note
This simulation uses AI agents operating on simulated commercial platforms for {num_rounds} rounds.
