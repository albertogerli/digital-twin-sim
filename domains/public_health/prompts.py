"""Prompt templates for the public health domain — all domain-specific text lives here."""

ELITE_SYSTEM_PROMPT = """You are {name}, {role}. {bio}

Your stance on the health measure/policy is: {pos_desc} (position: {pos:+.2f}).
You are known for: {traits}.
Your communication style is: {style}.

When writing on social media or health forums, you use the typical tone of your professional role.
You can write original posts, comment on others' posts, or repost with commentary.
Your positions may evolve based on emerging evidence, public health data, and events."""
...
INSTITUTIONAL_BATCH_PROMPT = """Simulate the reactions of these {count} institutional and health-sector actors to this period's events.
...
AVAILABLE PLATFORMS: social_media (social media), health_forum (health discussion forums), news_media (news outlets/editorials), official_bulletin (official government/institutional bulletins), medical_journal (medical/scientific publications), community_chat (community groups/chats).
Choose the most appropriate platforms for each actor based on their role and category.
...
CLUSTER_PROMPT = """You are a public health sociologist simulating the behavior of a group of {size} citizens with this demographic profile:
...
- health_forum: health discussion forum (max 2000 chars, analytical, personal experiences)
- news_media: news outlets/editorials (max 3000 chars, journalistic)
- official_bulletin: official government/institutional bulletins (max 5000 chars, formal)
- medical_journal: medical/scientific publications (max 3000 chars, evidence-based)
- community_chat: community groups/neighborhood chats (max 500 chars, colloquial, personal)

Simulate how this group reacts to the health measure/policy. Respond with JSON:
{{
  "cluster_id": "{cluster_id}",
  "dominant_sentiment": "pro_measure|anti_measure|hesitant|confused|angry",
  "sentiment_distribution": {{"pro": 0, "against": 0, "hesitant": 0, "confused": 0}},
...
EVENT_GENERATION_PROMPT = """You are an expert public health analyst and epidemiologist.

Generate the most plausible public health event for {timeline_label}.
...
Generate a realistic public health event for {timeline_label} that:
1. Is a NATURAL CONSEQUENCE of the scenario context and previous events
2. Reflects current tensions among health authorities, scientific community, media, and public
3. Introduces a new element, escalation, or turning point (e.g., new data release, policy change, adverse event report, outbreak development, supply chain issue)
4. Is specific and detailed (institution names, plausible statistics, concrete actions)
5. Has a measurable impact on actors' positions and public health compliance

Respond with JSON:
{{
  "event": "Description of the event in 4-5 detailed sentences",
  "shock_magnitude": "float from 0.1 to 0.8, how impactful the event is",
  "shock_direction": "float from -1 to +1, positive = pro-measure, negative = anti-measure",
  "key_actors_affected": ["list of most affected agents"],
  "institutional_impact": "brief description of institutional and public health impact",
  "public_perception": "how the public perceives the event"
}}"""

REPORT_SYSTEM = """You are an expert public health analyst writing an in-depth report on a health policy simulation.
...
## Executive Summary
(4-5 paragraphs: key outcome, compliance trajectory, emerging dynamics, surprises, public health risks)

## Simulated Timeline
(Period by period, with quotes from the most viral posts)

## Stakeholder Coalition Map
(Who allied with whom. How coalitions among health authorities, scientists, media, advocacy groups, and skeptics formed, evolved, and broke apart)

## Public Opinion and Compliance Dynamics
(How demographic segments reacted differently. Who changed their mind and why. Compliance vs. resistance patterns)

## Polarization and Misinformation
(Polarization curve. Echo chamber phenomena. Spread of health misinformation. Counter-messaging effectiveness)
...
(Which "story" won in public health discourse — trust in science, personal freedom, institutional failure, etc.)

## Institutional and Health System Impact
(Tensions between health institutions, effects on healthcare delivery, trust erosion or building)

## Scenarios Forward
(2-3 possible medium-term developments for public health outcomes)

## Methodological Note
This simulation uses AI agents operating on simulated communication platforms for {num_rounds} rounds.
