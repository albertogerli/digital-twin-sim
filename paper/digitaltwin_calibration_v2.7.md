...
**Version 2.7** — adds a benchmark-integrity layer addressing LLM data contamination in retrospective evaluation. A four-axis contamination probe quantifies how much prior knowledge the LLM used inside the simulator already carries about each empirical scenario before any dynamics are run, and a deterministic blinding protocol anonymizes titles, countries, dates, and agent names while preserving the numeric substrate (per-round support, shocks, initial positions, influence scores). Calibrated on the same 43-scenario corpus, the probe flags 11/43 high-leak political referendums (contamination index ≥ 0.60); applying the blinding protocol and re-probing drops their mean contamination index from 0.721 to 0.000 across all four axes (outcome, trajectory, events, actors), confirming that retrospective skill measured under blinding cannot be attributed to LLM memory. Section 6.10 reports the full A/B result; Appendix F documents the probe, the blinding transformations, and the reproducibility package. All v2.6 calibration, benchmark, and EnKF numbers remain unchanged — the contamination layer is evaluation infrastructure, not a change to the dynamics model.

**Version 2.6** — adds a pre-simulation roster-realism layer (scope analyzer + realism gate with enforcement and composition control) that constrains LLM-generated agent selection to be in-scope with respect to sector and geography. The v2.5 calibration, benchmarking, and EnKF numbers are unchanged — Layer 0 operates upstream of the opinion-dynamics model, on the composition of the agent roster, not on the dynamics of opinion update. Section 6.9 reports the realism-benchmark results on two pilot briefs; Appendix E documents the Layer 0 pipeline.
...
#### 6.9.1 Corpus-Wide Audit on the 44-Scenario Empirical Set
...
The aggregate accept rate is 88.7 % (172 of 194 named elite + institutional agents across 43 in-scope scenarios; one corpus manifest file was correctly skipped). The per-domain breakdown reveals a sharp split:
...
13. **Sim-vs-baseline on blinded empirical scenarios.** Section 6.10 establishes that LLM prior knowledge on the 11 high-leak political scenarios collapses from index 0.721 to 0.000 under blinding, making retrospective skill on the blinded variants interpretable as genuine simulation output. The remaining v2.7 deliverable is to run the simulator itself on each of the 11 scenarios under both arms (contaminated and blinded), compute the full trajectory-comparison row from `benchmarks/eval_metrics.py` (DTW, normalized DTW, KS, RMSE, terminal error) against ground truth, and emit Diebold–Mariano rows of the simulator vs. each null baseline from Section 6.6. The expected compute budget is 22 full sim runs at \$1–3 and 10–30 min each; a checkpointed runner is needed because sequential execution would take 4–8 h wall-clock at concurrency 2. The decisive result is whether the simulator's blinded-arm DTW and terminal-error remain comparable to its contaminated-arm counterparts and whether both beat the Section 6.6 baselines at $p < 0.05$ on $\geq 6 / 11$ scenarios. A positive result establishes dynamics-as-the-source-of-skill for the retrospective benchmark; a negative result bounds how much of the v2.5 predictive-skill story on political referendums was attributable to LLM memory and would motivate a re-weighting of the corpus toward domains with intrinsically lower contamination (Boeing MAX, SVB, Dieselgate, Archegos — all already at index $< 0.35$ in Table 20's full output).

14. **Residual-leak closure on event descriptions.** The current blinding protocol (Section 6.10.2) cleans title, country, dates, and agent names; event-description scrubbing relies on the agent rename map, the multi-token expansion, and a $\sim 50$-entry stoplist, which together catch scenario-specific principals but leave behind non-agent proper nouns that the LLM could in principle use to hook memory (e.g., the name of a former PM referenced in an event but not in the agent roster). The contamination probe's numeric signal is unaffected by this residual because the LLM never receives the event descriptions during probing — only title, country, and date. For sim-side runs, however, the agents *will* see event descriptions, so a secondary LLM-scrub pass on those descriptions (replace residual proper nouns with generic placeholders, cached deterministically per scenario) is required before the Section 8.3(13) run is considered tight. Budget: one extra LLM call per event $\approx$ 500 calls corpus-wide, \$0.05 one-time.
