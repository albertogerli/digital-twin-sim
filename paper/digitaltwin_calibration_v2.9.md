# A Calibrated LLM-Conditioned Agent-Based Model of Public Opinion Dynamics with Null-Baseline Benchmarking, Data-Contamination Auditing, Online Assimilation, and a Power-Law Economic-Impact Estimator
...
**Keywords:** opinion dynamics, agent-based modeling, large language models, Bayesian calibration, Ensemble Kalman filter, computational social science, Diebold–Mariano test, null baselines, data contamination, blinding protocol, DORA economic impact, power-law cost model, regime-switching HMM, bootstrap quantile inference

**Version 2.9** — adds a calibrated economic-impact estimator (Section 9) for DORA major-incident reporting, with a power-law primary model (cost = β·s^γ) validated by leave-one-out cross-validation against a curated table of N=40 historical incidents 1998–2024.
...
Version 2.9 extends the framework with a calibrated **economic-impact estimator** (Section 9) that maps simulated shock magnitudes to euro losses for DORA major-incident reporting. We curate a public-domain reference table of N=40 historical incidents (1998–2024) across seven categories — Italian banking, EU banking, US banking, sovereign, cyber, telco, energy — and fit a power-law cost model $\hat{c}(s) = \beta \cdot s^\gamma$ per category by log-log Huber regression. Leave-one-out cross-validation puts hit-rate within $\pm 100\%$ at 80% under the power-law model, against 35% for a linear $\hat{c}(s)=\alpha s$ baseline; the linear model is reported alongside for transparency. The estimator surfaces six independent diagnostic layers: (i) a 5000-replicate pairs-bootstrap on (β, γ) for the epistemic interval, (ii) the Eicker–Huber–White–MacKinnon HC3 sandwich standard error, (iii) a 2-state Gaussian hidden Markov model on log(VIX) monthly 1997–2025 for incident-level regime posteriors, (iv) a Hill estimator for the Pareto tail index of residuals, (v) two-stage least squares with the regime posterior as instrument for shock_units, and (vi) the log-log slope as a Taleb-style fragility exponent. Headline numbers and uncertainty bands track the production estimator end-to-end through the public `/compliance` interface.
...
10. **A calibrated economic-impact estimator** (Section 9) that converts simulated shock magnitudes into euro cost estimates for DORA major-incident reporting. We fit a power-law primary model $\hat{c}(s) = \beta \cdot s^\gamma$ per category by log-log Huber regression on a public-domain table of N=40 historical incidents (1998–2024). Leave-one-out cross-validation hit-rate within $\pm 100\%$ is 80% under the power-law model versus 35% for a linear $\hat{c}(s)=\alpha s$ baseline; both models run live, with the linear baseline retained for transparency. Six independent diagnostic layers — pairs-bootstrap for the epistemic interval, HC3 heteroscedastic-robust SE, a 2-state Gaussian HMM on log(VIX) monthly 1997–2025 with per-incident regime posteriors, a Hill estimator for the Pareto tail index, 2SLS with the HMM regime posterior as instrument for shock_units, and a Taleb fragility exponent — surface the assumptions and limits behind the headline number to the chief risk officer rather than hiding them.
...
## 9. Economic-Impact Estimator: From Hardcoded Constant to Calibrated Power-Law

DORA (Digital Operational Resilience Act, EU Regulation 2022/2554, RTS Annex I) requires regulated financial entities to report the economic impact of major ICT-related incidents within deadlines defined by a seven-criterion classification matrix. The corresponding line in our compliance pipeline — `economic_impact_eur` — is the headline number on the `/compliance` interface and the entire DORA application of the framework hinges on it being defensible, traceable, and honest about its uncertainty. This section documents the calibration pipeline that produced the v2.9 estimator, the decision to promote a power-law cost model from diagnostic to primary headline, and the six independent statistical layers that surface the assumptions and limits behind that headline number.

### 9.1 Motivation

The v2.7 implementation hardcoded `economic_impact_eur = total_shock_units × 50,000,000` (€50M per simulated shock-unit). This had three problems: (i) the €50M constant was an unmotivated guess; (ii) there was no propagation of uncertainty to the chief risk officer who reads the number; (iii) the linear functional form $\hat{c}(s) = \alpha s$ does not match the empirical regularity that cost responds super-linearly to shock magnitude in financial-system events.

We close all three with the v2.9 estimator: (i) replace the €50M constant with a Huber-fitted slope on a curated public-domain reference table; (ii) report epistemic intervals from a 5000-replicate pairs-bootstrap and provide six diagnostic layers; (iii) promote a log-log-fitted power-law $\hat{c}(s) = \beta \cdot s^\gamma$ to the primary headline whenever its log-space $R^2 \geq 0.5$, with the linear $\alpha s$ baseline retained alongside for transparency.

### 9.2 Reference Incident Table

The calibration target is `shared/dora_reference_incidents.json` (schema v1.1), a curated table of $N=40$ historical incidents 1998–2024 across seven categories — `banking_it` (Italian banks, $n=6$), `banking_eu` ($n=8$), `banking_us` ($n=7$), `sovereign` ($n=6$), `cyber` ($n=6$), `telco` ($n=4$), `energy` ($n=3$). Each row carries: an incident identifier; the simulated shock_units value; the public-domain euro cost in millions; the category; an ISO incident date (added in v1.1 for HMM/IV machinery); a hand-coded regime label (`calm` / `stressed` / `crisis`); a free-text label; and at least two source citations (regulator filings, financial-stability reports, or peer-reviewed retrospectives). Examples: MPS precautionary recapitalisation 2017 ($s=1.6$, $c=$€3.9B; sources: DG Comp SA.47677, MEF press release 2017-07-04); Lehman Brothers bankruptcy 2008 ($s=4.0$, $c=$€600B; sources: FCIC 2011 final report, BIS Quarterly Review Q4 2008); CrowdStrike global outage 2024 ($s=2.8$, $c=$€10B; sources: Microsoft + CrowdStrike joint post-mortem, Delta Air Lines 2024 Q3 8-K). All 40 sources are inspectable in the JSON; the corpus is auditable and version-controlled. Shock_units estimates fall into four heuristic tiers — 0.5–1.2 single-firm contained, 1.2–2.0 sector-wide moderate, 2.0–3.0 major sector or sovereign-adjacent, $\geq 3.0$ systemic — and the operator can refine any one by re-running the corresponding incident through the simulator and measuring $\sum |s_t \cdot \hat{d}_t|$ empirically (`scripts/replay_historical_incident.py`).

### 9.3 Linear Baseline: Huber α with Bootstrap Quantiles and HC3 Sandwich SE

The first calibration layer is a Huber-fitted slope $\hat{\alpha}$ on the linear no-intercept model
$$
c_i = \alpha \cdot s_i + \varepsilon_i, \quad i = 1, \ldots, N
$$
with the iteratively reweighted least squares (IRLS) Huber loss at the standard $k=1.345$ tuning constant, which gives 95% asymptotic efficiency under Gaussian errors while bounding the influence of large residuals. The fit is computed jointly in three slicings of the reference table — overall, by category, and by category × regime — and the most specific slicing with $\geq 3$ observations is used at request time. Per-category $\hat{\alpha}$ ranges from €1.95B/unit (`banking_it`, $R^2 = 0.88$) to €65.5B/unit (`banking_us`, $R^2 = 0.46$, dominated by Lehman); the overall fit gives $\hat{\alpha} = $€14.0B/unit, $R^2 = 0.20$ — a low value driven entirely by Lehman's 5σ residual.

The reported uncertainty band on $\hat{\alpha}$ is the empirical 5°/95° quantile of the bootstrap distribution from $B=5000$ pairs-bootstrap replicates on the calibration subset. This replaces the v2.7 placeholder $\pm 1.645 \sigma$ Gaussian band, which assumed both normality and homoscedasticity of the residual sequence — neither holds in the data. The pairs-bootstrap propagates joint uncertainty in (i) the coefficient estimate and (ii) the residual variance through to the cost prediction at the request's target shock. Alongside the bootstrap quantiles we report the Eicker–Huber–White–MacKinnon HC3 sandwich estimator
$$
\widehat{\text{Var}}_{\text{HC3}}(\hat{\alpha}) = \frac{1}{(\sum_i s_i^2)^2} \sum_i s_i^2 \left( \frac{c_i - \hat{\alpha} s_i}{1 - h_i} \right)^2, \quad h_i = \frac{s_i^2}{\sum_j s_j^2}
$$
where $h_i$ is the leverage of observation $i$. HC3 inflates standard errors in the presence of heteroscedasticity (residuals scaling with the regressor) without requiring the analyst to specify the variance form; the difference $\hat{\sigma}_{\text{HC3}} - \hat{\sigma}_{\text{OLS}}$ is itself a heteroscedasticity diagnostic. On the `banking_it × stressed` slice we measure $\hat{\sigma}_{\text{HC3}} =$€404M/unit against the homoscedastic $\hat{\sigma} =$€1089M/unit, indicating that residuals are *less* dispersed than a homoscedastic-Gaussian model would suggest — that is, a few high-leverage observations were inflating the homoscedastic estimate.

### 9.4 Power-Law Primary: $\hat{c}(s) = \beta \cdot s^\gamma$

The linear model assumes that doubling the shock magnitude doubles the cost. The reference table contradicts this: a log-log Huber fit on the overall corpus produces $\hat{\gamma} = 3.36$ ($R^2_{\text{log}} = 0.72$, $N = 40$), and on the `banking_it` slice $\hat{\gamma} = 3.12$ ($R^2_{\text{log}} = 0.97$, $n = 6$) — i.e., doubling shock units approximately octuples the expected cost. Linear $\alpha s$ cannot capture this convexity; it systematically under-predicts large shocks (Lehman: linear says €56B, actual €600B) and over-predicts small ones (Tercas 2014: linear says €1.56B, actual €300M).

The v2.9 production estimator therefore adopts the power-law model
$$
\hat{c}(s) = \beta \cdot s^\gamma, \quad \log c_i = \log \beta + \gamma \log s_i + u_i
$$
fit by ordinary least squares in log space (Huber loss adds little when residuals are nearly Gaussian post-log-transformation). The coefficient $\gamma$ is exactly the Taleb fragility exponent of §9.5(vi); $\beta$ is the cost in EUR-millions at $s=1$. The headline point estimate is $\hat{\beta} \cdot s_*^{\hat{\gamma}}$, where $s_*$ is the simulator's $\sum_t |s_t \cdot \hat{d}_t|$ aggregate shock for the request's brief. The epistemic interval is the empirical 5°/95° quantile of $B=5000$ replications of $\beta_b \cdot s_*^{\gamma_b}$, where $(\beta_b, \gamma_b)$ are pairs-bootstrap refits. This propagates joint $(\beta, \gamma)$ uncertainty — which is correlated, since $\log \beta$ is the intercept and $\gamma$ the slope of the log-log fit — through to the prediction.

The model selection rule: power-law is promoted to primary when $R^2_{\text{log}} \geq 0.5$ on the calibration subset; otherwise the linear $\alpha s$ baseline is used. The 0.5 threshold reflects a pragmatic test that the log-log fit is meaningfully better than a horizontal line in log-cost space. We additionally flag *extrapolation*: when the request's shock exceeds $1.3 \times \max_i s_i$ on the training subset, we surface a warning in the UI ("target shock is $r$× the historical max"). Power laws are well-known to be hazardous outside their support, and the $1.3$ threshold is conservative — Lehman 2008 ($s=4.0$) is at the right tail of the `banking_us` bucket and predictions for $s > 5$ in the same bucket would be reported with the warning prominent.

#### 9.4.1 Concrete validation against the reference table

For three within-bucket reference points in `banking_it × stressed`:

| Incident | $s$ | actual $c$ (€M) | linear pred | power-law pred |
|---|---|---|---|---|
...
The linear model fails systematically on Tercas (small-shock contained event); the power-law splits its errors symmetrically around the truth. For the genuine tail event (Lehman $s = 4.0$, actual €600B), the power-law predicts €422B (−30%) while linear predicts €199B (−67%); both under-predict the true 6σ shock, but the power-law captures more of the convexity.

### 9.5 Diagnostic Overlay: Six Independent Layers

The headline number is one quantity, computed under one set of assumptions. To avoid the false certainty of a single black-box estimator, we surface six independent diagnostics alongside the headline. Each carries its own uncertainty quantification and is designed so that a chief risk officer can read the headline number against the independent diagnostic and identify which assumption is doing the work.

**(i) Pairs-bootstrap quantile band, §9.3.** Empirical 5°/95° quantiles from $B=5000$ resamples — captures joint coefficient and residual uncertainty without normality assumption.

**(ii) HC3 heteroscedastic-robust SE, §9.3.** Eicker–Huber–White–MacKinnon sandwich estimator; the difference from the homoscedastic-Gaussian σ is a heteroscedasticity diagnostic.

**(iii) Hidden Markov regime posterior, this section.** A 2-state Gaussian HMM is fit on log(VIX) monthly observations 1997-01 through 2025-12 ($T=348$ observations, fitted via Baum–Welch / forward–backward in log-space). The two estimated regimes are well-separated and persistent: low-vol regime $\hat{\mu}_0 = \log(14.7)$, high-vol regime $\hat{\mu}_1 = \log(24.9)$, with $P(z_{t+1} = z_t) \approx 0.96$ in both states (mean dwell time $\sim 25$ months). Each reference incident inherits a regime posterior $p_i = P(z_i = \text{high} | x_{1:T})$ from the smoother evaluated at the incident's calendar month. We then fit a regime-mixture α via two-feature OLS:
$$
c_i = \alpha_{\text{low}} (1 - p_i) s_i + \alpha_{\text{high}} p_i s_i + \nu_i.
$$
The empirical regime amplification on the overall corpus is $\hat{\alpha}_{\text{high}} / \hat{\alpha}_{\text{low}} = 20.1$ ($\hat{\alpha}_{\text{low}} = $€2.4B/unit, $\hat{\alpha}_{\text{high}} = $€47.7B/unit, $R^2 = 0.43$). This refines the hand-coded calm/stressed/crisis label: Cyprus 2013 (hand-labeled `crisis`, $p_i = 0.000$ — VIX was actually low at the time, the crisis was sovereign-debt regional rather than global vol), Long-Term Capital Management 1998 (hand-labeled `stressed`, $p_i = 1.000$ — the joint Russian-default-plus-LTCM unwind was a global vol event). The mixture posterior is more honest than the binary label because it inherits the data-driven regime.

**(iv) Hill estimator for the Pareto tail index of |residuals|, this section.** For the top $k = \lfloor 0.10 N \rfloor$ residuals ranked by absolute value,
$$
\hat{\alpha}_{\text{Hill}} = \frac{1}{(1/k) \sum_{i=1}^{k} \log r_{(i)} - \log r_{(k+1)}}.
$$
On the overall corpus we measure $\hat{\alpha}_{\text{Hill}} = 0.74$ — formally an infinite-variance regime, dominated by Lehman. On the `banking_it` slice we measure $\hat{\alpha}_{\text{Hill}} = 4.91$ — moderate tail. The diagnostic surfaces a red flag in the UI when $\hat{\alpha}_{\text{Hill}} < 2$ (infinite variance), telling the operator that point estimates are still meaningful but the upper tail of the cost distribution is genuinely unbounded.

**(v) 2SLS-IV with HMM regime as instrument, this section.** The simulated `shock_units` quantity is plausibly endogenous to sim calibration: if the sim were tuned on the same outcomes the costs are derived from, OLS on the reference table would over-estimate the slope. We instrument $s_i$ with the HMM regime posterior $p_i$ — exogenous to the sim by construction (it is derived from VIX, an external observation channel). The just-identified Wald estimator gives $\hat{\beta}_{\text{2SLS}} = \widehat{\text{Cov}}(p_i, c_i) / \widehat{\text{Cov}}(p_i, s_i)$. We report the first-stage F-statistic alongside; on the current $N=40$ corpus $F \approx 0.9$, well below the Stock–Yogo $F \geq 10$ threshold for valid weak-instrument inference. The IV is therefore **published as a diagnostic, not as the headline**: the regime posterior is too weak an instrument, on this sample size, to rule out endogeneity bias. The honest framing for the chief risk officer is "we tested for sim-calibration endogeneity; the only available regime-derived instrument has $F < 1$, so we cannot definitively rule it out; the remedy is more reference incidents annotated with both shock_units and an independent stress measurement."

**(vi) Taleb fragility exponent, this section.** The log-log slope $\hat{\gamma}$ from §9.4 is identical to Taleb's fragility exponent: $\gamma > 1$ flags convex / fragile exposure, $\gamma < 1$ concave / antifragile, $\gamma \approx 1$ linear. On the overall corpus we measure $\hat{\gamma} = 3.36$ ($R^2_{\text{log}} = 0.72$, $N = 40$); on `banking_it` $\hat{\gamma} = 3.12$ ($R^2 = 0.97$, $n = 6$). Both signal strongly convex exposure. We surface this as a chip in the UI ("fragile γ=3.12") whenever $\hat{\gamma} > 1.10$, telling the operator that the linear $\alpha s$ baseline is mis-specified and the power-law primary is doing the work.

### 9.6 Validation: Leave-One-Out Cross-Validation in Three Modes

The validation panel on `/compliance` reports leave-one-out cross-validation under three increasingly realistic models. For each held-out incident $i$, the estimator is refit on the remaining $N-1$ rows under the chosen model, and the held-out cost is predicted; aggregates report hit-rate within $\pm 50\% / \pm 100\% / \pm 200\%$, MAE, RMSE, and median absolute percent error.

| Model | Hit ±50% | Hit ±100% | Hit ±200% | Median \|err\| | MAE |
|---|---|---|---|---|---|
| `overall` (linear, single α across N=40) | 20% | 35% | 40% | 394% | €31.9B |
| `category_aware` (linear $\alpha s$, refit per category) | 20% | 40% | 52% | 166% | €43.3B |
| **`power_law`** (β·s^γ, refit per category) — production | **48%** | **80%** | **88%** | **57%** | **€25.7B** |

Three observations. First, naive `overall` linear fit has hit-rate $\pm 100\%$ at only 35%. The reason is illustrated by `Tercas 2014` ($s = 0.8$, actual €300M): the LOO refit on the other 39 rows gives $\hat{\alpha} \approx $€14B/unit (heavily influenced by Lehman and other banking_us observations), and the prediction for Tercas is $0.8 \times $€14B$ = $€11.3B — wildly over-predicted by a factor of 38. Second, going to `category_aware` (per-category fit) drops the median error from 394% to 166% but still leaves over half the corpus outside $\pm 100\%$, because the linear functional form is wrong. Third, `power_law` clears 80% of the corpus to within $\pm 100\%$ — this is the production estimator's defensible empirical claim.

The remaining errors in `power_law` mode concentrate in the `banking_us` 2023 sub-bucket — Silicon Valley Bank ($s = 2.4$, actual €9B; predicted €40B), Signature Bank, First Republic — where the per-category $\hat{\gamma}$ is pulled high by Lehman 2008 ($s = 4.0$, actual €600B). This is a sample-size issue (only 7 banking_us incidents, of which one is a 5σ outlier), not a model-form issue. The LOO pipeline flags the residual; the remediation path is annotating more banking_us incidents from the 2014–2019 calm-regime period (Wells Fargo cross-selling 2016, JP Morgan London Whale 2012, Goldman Sachs 1MDB 2018, etc.) to balance the bucket.

### 9.7 Production Wiring and UI Surfacing

The estimator is exposed end-to-end through the public `/compliance` interface. The DORA Export panel shows the headline `economic_impact_eur` as a euro figure with appropriate magnitude formatting (€M / €B / €T), color-coded against four severity tiers (low / medium / high / critical) tied to the four DORA reporting thresholds. The figure carries the active-model badge ("power-law β·s^γ" green when promoted, "linear α·s" gray when fallback) and the epistemic interval (formatted as "Epistemic range [low – high] (n=40)") with a tooltip that reproduces the full bootstrap configuration. Below the headline, a methodology drawer can be expanded; it shows two ranked blocks (active power-law in green, linear baseline in gray for transparency), then the six diagnostic chips (bootstrap-q90, fragile γ, heavy tail α, regime ×N, IV F, HC3 SE) each with hover help.

A separate DORA Validation tab shows the leave-one-out backtest table, with the per-row "fit_scope" surfacing which subset trained the prediction (e.g., `banking_it` for an MPS-class incident, `overall (fallback)` for an `energy` incident with $n=3 < 4$ in-bucket). The mode toggle (power_law / category_aware / overall) lets the chief risk officer see why the production system uses the power-law: clicking from `power_law` to `overall` immediately displays the 80% → 35% hit-rate drop. This is the framework's answer to the standard CRO question "how do you know your method works": every claim is tied to an inspectable backtest configuration that the operator can reconstruct.

The full pipeline, including category-detection from brief text via keyword scoring, the $5000$-replicate bootstrap, the HMM regime fit, the HC3 sandwich estimator, and the LOO cross-validation, runs in $\sim 2$ s for a typical request — well below the timing budget of a DORA report-generation step.

### 9.8 Limits and Honest Framing

Three limits to surface explicitly. First, the reference table is small: $N=40$ across 7 categories, with the smallest bucket (`energy`) at $n=3$. Per-category $\hat{\gamma}$ has wide bootstrap confidence intervals on the smaller buckets, and the production system falls back to `overall` (fallback) when $n < 4$ in the brief's category. We are actively expanding the corpus toward $N \geq 60$ for v3.0, which will tighten the per-category intervals. Second, the IV is currently weak ($F < 1$); without a stronger instrument we cannot definitively bound endogeneity bias, only acknowledge it. Third, the Hill estimator on the overall corpus indicates an infinite-variance regime: the estimator's *point* prediction is well-identified, but its *upper tail* is genuinely unbounded — the chief risk officer should read the headline as a central tendency, not a worst case. The correct worst case is the 95° quantile of the bootstrap, which on Lehman-class shocks is roughly twice the point.

What we have not yet built: (i) a hierarchical Bayesian extension of the cost model that pools information across categories with a per-category random effect on $\gamma$, which would reduce the Lehman-effect on the `banking_us` bucket without dropping Lehman; (ii) an LLM-judge layer (Sprint C, scaffolded) that reasons over the brief plus the $K$ most-similar reference incidents (RAG retrieval) and produces a `gemini-3.1-pro-preview` cost estimate as a third independent point, blended by confidence; (iii) the formal cost-model power-law switch with regime-conditional $(\beta, \gamma)$ pairs (combining §9.4 and §9.5(iii)). Each is an extension of the same calibration target rather than a replacement.
...
The v2.9 economic-impact estimator (Section 9) is the framework's first compliance-grade application: a calibrated mapping from the simulator's aggregate shock units to euro losses for DORA major-incident reporting. Its principal empirical finding — that the cost surface is convex with $\hat{\gamma} \approx 3$ on the $N=40$ corpus, and that promoting a power-law $\hat{c}(s) = \beta \cdot s^\gamma$ from diagnostic to primary headline lifts leave-one-out hit-rate within $\pm 100\%$ from 35% to 80% — is itself a contribution to the operational-risk literature, not just a feature of our particular sim. The six diagnostic layers (bootstrap quantiles, HC3 SE, HMM regime mixture, Hill tail index, 2SLS-IV, fragility exponent) are designed so that the chief risk officer reads the headline number alongside its assumptions, not against a black box. The estimator is the cleanest demonstration that the framework's value lies in the instrumentation around the simulator — calibration, validation, contamination audit, regime conditioning, fragility quantification — at least as much as in the simulator itself.

---

<!-- ================================================================== -->
<!--  CUT LINE — Main manuscript ends here.                              -->
<!--  Sections 1-10 above constitute the JASSS main submission.          -->
